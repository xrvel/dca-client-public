{{-- resources/views/mydcaschedules/edit.blade.php --}}
@extends('layouts.app')
...
@section('content')
	<div class="container mt-4">
		<h1 class="mb-4">Edit DCA Schedule</h1>

		<form action="{{ route('dcaschedules.update', $dcaschedule->id) }}" method="POST">
			@csrf
			@method('PUT')
			...
			<div class="mb-3">
				...
				<input type="text" class="form-control @error('label') is-invalid @enderror" id="label" name="label"
					value="{{ old('label', $dcaschedule->label) }}" required autofocus placeholder="Enter label for the schedule">
				@error('label')
			...
			<div class="mb-3">
				...
				<input type="text" class="form-control @error('pair_name') is-invalid @enderror" id="pair_name" name="pair_name"
					value="{{ old('pair_name', $dcaschedule->pair_name) }}" required placeholder="e.g., BTCUSDT">
				@error('pair_name')
					<div class="invalid-feedback">{{ $message }}</div>
				@enderror
			</div>
			...
			<div class="mb-3">
				...
				@if ($dca_keys->isEmpty())
					<div class="alert alert-warning" role="alert">
						...
						<a href="{{ route('dca_keys.create') }}" class="btn btn-link">Create Exchange Key</a>
					</div>
				@else
					<select class="form-select @error('key_id') is-invalid @enderror" id="key_id" name="key_id" required>
						...
						@foreach ($dca_keys as $item)
							<option value="{{ $item->id }}" @selected(old('key_id', $dcaschedule->key_id) == $item->id)>{{ $item->label }} - {{ $item->exchange_name }}
								- {{ $item->id }}</option>
						@endforeach
					</select>
				@endif
			...
			<div class="mb-3">
				...
				<select class="form-select @error('buy_strategy') is-invalid @enderror" id="buy_strategy" name="buy_strategy"
					...
					<option value="linear" @selected(old('linear', $dcaschedule->buy_strategy) == 'linear')>Dynamic Linear</option>
					<option value="log_1" @selected(old('buy_strategy', $dcaschedule->buy_strategy) == 'log_1')>Dynamic Logarithm 1</option>
					<option value="log_2" @selected(old('buy_strategy', $dcaschedule->buy_strategy) == 'log_2')>Dynamic Logarithm 2 (aggressive)</option>
					<option value="log_low_1" @selected(old('buy_strategy', $dcaschedule->buy_strategy) == 'log_low_1')>Dynamic Logarithm - Low Threshold 1</option>
					<option value="square_1" @selected(old('buy_strategy', $dcaschedule->buy_strategy) == 'square_1')>Squared 1 (aggressive)</option>
				</select>
				@error('buy_strategy')
					<div class="invalid-feedback">{{ $message }}</div>
				@enderror
			...
			<div class="mb-3">
				...
				<input type="number" class="form-control @error('base_amount') is-invalid @enderror" id="base_amount"
					name="base_amount" value="{{ old('base_amount', $dcaschedule->base_amount) }}" required
			...
			<div class="mb-3" id="scheduled_every_group">
				...
				<select class="form-select @error('scheduled_every') is-invalid @enderror" id="scheduled_every"
					...
					<option value="everyMinute" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'everyMinute')>Every 1 Minute</option>
					<option value="everyFiveMinutes" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'everyFiveMinutes')>Every 5 Minutes</option>
					<option value="everyTenMinutes" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'everyTenMinutes')>Every 10 Minutes</option>
					<option value="everyFifteenMinutes" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'everyFifteenMinutes')>Every 15 Minutes</option>
					<option value="everyThirtyMinutes" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'everyThirtyMinutes')>Every 30 Minutes</option>
					<option value="hourly" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'hourly')>Every 1 Hour</option>
					<option value="everyOddHour" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'everyOddHour')>Every Odd Hours</option>
					<option value="everyTwoHours" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'everyTwoHours')>Every 2 Hours</option>
					<option value="everyThreeHours" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'everyThreeHours')>Every 3 Hours</option>
					<option value="everyFourHours" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'everyFourHours')>Every 4 Hours</option>
					<option value="everySixHours" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'everySixHours')>Every 6 Hours</option>
					<option value="daily" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'daily')>Daily</option>
					<option value="weekly" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'weekly')>Weekly</option>
					<option value="monthly" @selected(old('scheduled_every', $dcaschedule->scheduled_every) == 'monthly')>Monthly</option>
					<option value="custom" @selected(old('custom', $dcaschedule->scheduled_every) == 'custom')>Custom</option>
				</select>
			...
			<div class="mb-3" id="scheduled_every_option_group">
				...
				<input type="text" class="form-control @error('scheduled_every_option') is-invalid @enderror"
					id="scheduled_every_option" name="scheduled_every_option"
					value="{{ old('scheduled_every_option', $dcaschedule->scheduled_every_option) }}"
			...
			<div class="mb-3">
				...
				<select class="form-select @error('risk_symbol') is-invalid @enderror" id="risk_symbol" name="risk_symbol"
					...
					<option value="ETH" @selected(old('risk_symbol', $dcaschedule->risk_symbol) == 'ETH')>ETH</option>
					<option value="BNB" @selected(old('risk_symbol', $dcaschedule->risk_symbol) == 'BNB')>BNB</option>
					<option value="SOL" @selected(old('risk_symbol', $dcaschedule->risk_symbol) == 'SOL')>Solana</option>
					<option value="ADA" @selected(old('risk_symbol', $dcaschedule->risk_symbol) == 'ADA')>Cardano</option>
					<option value="XRP" @selected(old('risk_symbol', $dcaschedule->risk_symbol) == 'XRP')>XRP</option>
				</select>
				@error('risk_symbol')
					<div class="invalid-feedback">{{ $message }}</div>
				@enderror
			...
			<div class="mb-3">
				...
				<input type="number" step="0.01" class="form-control @error('min_risk_buy') is-invalid @enderror"
					id="min_risk_buy" name="min_risk_buy" value="{{ old('min_risk_buy', $dcaschedule->min_risk_buy) }}" required
			...
			<div class="mb-3">
				...
				<input type="number" step="0.01" class="form-control @error('max_risk_buy') is-invalid @enderror"
					id="max_risk_buy" name="max_risk_buy" value="{{ old('max_risk_buy', $dcaschedule->max_risk_buy) }}" required
					placeholder="e.g., 25.5">
				@error('max_risk_buy')
					<div class="invalid-feedback">{{ $message }}</div>
				@enderror
			...
			<div class="mb-3">
				...
				<input type="number" class="form-control @error('min_buy_amount') is-invalid @enderror" id="min_buy_amount"
					name="min_buy_amount" value="{{ old('min_buy_amount', $dcaschedule->min_buy_amount) }}" required
			...
			<div class="mb-3">
				...
				<input type="number" class="form-control @error('max_buy_amount') is-invalid @enderror" id="max_buy_amount"
					name="max_buy_amount" value="{{ old('max_buy_amount', $dcaschedule->max_buy_amount) }}" required
			...
			<div class="mb-3">
				...
				<textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
				 rows="3" placeholder="Optional description...">{{ old('description', $dcaschedule->description) }}</textarea>
				@error('description')
					<div class="invalid-feedback">{{ $message }}</div>
				@enderror
			</div>
			...
			<div class="mb-3">
				...
				<select class="form-select @error('is_active') is-invalid @enderror" id="is_active" name="is_active" required>
					<option value="1" @selected(old('is_active', $dcaschedule->is_active) == 1)>Yes</option>
					<option value="0" @selected(old('is_active', $dcaschedule->is_active) == 0)>No</option>
				</select>
				@error('is_active')
					<div class="invalid-feedback">{{ $message }}</div>
				@enderror
				...
				<div class="card-header">
					<h5 class="mb-0">
						<input type="checkbox" class="form-check-input me-2" id="enable_extra_buys" name="enable_extra_buys"
							value="1" @checked(old('enable_extra_buys', $dcaschedule->enable_extra_buys))>
					...
					<div class="form-text">
						...
						reset the extra buys counter on the predefined interval.
					</div>
				</div>
				<div class="card-body" id="extra_buys_settings">
					<!-- Current Extra Buys Status -->
					@if ($dcaschedule->enable_extra_buys)
						<div class="alert alert-info mb-3">
							<strong>Current Extra Buys Status:</strong><br>
							• Executed: {{ $dcaschedule->extra_buys_executed_count }} / {{ $dcaschedule->max_extra_buys_per_interval }}<br>
							• Reset Interval: {{ ucfirst($dcaschedule->extra_buys_reset_interval) }}<br>
							• Reset Mode: {{ $dcaschedule->reset_mode == 1 ? 'Relative' : 'Absolute' }}<br>
							@if ($dcaschedule->last_extra_buy_timestamp > 0)
								• Last Extra Buy: {{ date('Y-m-d H:i:s', $dcaschedule->last_extra_buy_timestamp) }}
							@else
								• Last Extra Buy: Never
							@endif
						</div>
					@endif
						...
						<div class="col-md-6">
							<div class="mb-3">
								...
								<select class="form-select @error('extra_buys_reset_interval') is-invalid @enderror"
									...
									<option value="daily" @selected(old('extra_buys_reset_interval', $dcaschedule->extra_buys_reset_interval) == 'daily')>Daily</option>
									<option value="weekly" @selected(old('extra_buys_reset_interval', $dcaschedule->extra_buys_reset_interval) == 'weekly')>Weekly</option>
									<option value="monthly" @selected(old('extra_buys_reset_interval', $dcaschedule->extra_buys_reset_interval) == 'monthly')>Monthly</option>
								</select>
						...
						<div class="col-md-6">
							<div class="mb-3">
								...
								<input type="number" class="form-control @error('max_extra_buys_per_interval') is-invalid @enderror"
									...
									value="{{ old('max_extra_buys_per_interval', $dcaschedule->max_extra_buys_per_interval) }}" min="0"
									max="100" placeholder="e.g., 1">
						...
						<div class="col-md-6">
							<div class="mb-3">
								...
								<input type="number" class="form-control @error('min_hours_between_extra_buys') is-invalid @enderror"
									...
									value="{{ old('min_hours_between_extra_buys', $dcaschedule->min_hours_between_extra_buys) }}" min="1"
									max="168" placeholder="e.g., 1">
