...
<ol>
	<li>
		...
		@else
		<span class="badge text-bg-danger">Invalid</span>, check <code>ALPHASQUARED_API_KEY</code> in your <code>.env</code> file.
		@endif
	</li>
	<li>
		Exchange Keys :
		@if ($dca_key_count > 0)
		<span class="badge text-bg-success">OK</span> : <a href="{{ route('dcakeys.index') }}">{{ number_format($dca_key_count) }} keys</a> detected.
		@else
	...
	<li>
		...
		<span class="badge text-bg-success">OK</span> : <a href="{{ route('dcaschedules.index') }}">{{ number_format($dca_schedule_count) }} schedules</a> detected.
		@else
	...
	<li>
		...
		<ol>
			<li>have set <code>DCA_BINANCE_PROXY_URL</code> in <code>.env</code> (current remote proxy URL is <code>{{ env('DCA_BINANCE_PROXY_URL') }}</code>) and</li>
			<li>have proxy file in your server</li>
		</ol>
		If you want to connect to Binance, and your computer (or your server) is not supported on Binance supported countries, you can place a custom file in Binance supported countries to act as a &quot;proxy&quot; or &quot;hop&quot; between your computer / server to Binance server.<br />
		...
		fetch('<?php echo route('api.risk.get'); ?>', {
			...
			.then(async (response) => {
				...
				if (!response.ok) {
					...
					resultDiv.innerHTML = `<div class="alert alert-danger">Error ${response.status}: ${errorText}</div>`;
					return;
				}
